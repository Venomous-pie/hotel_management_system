...
export function buildReservationSuccessMessage(args: {
  ...
  const guestFirst = reservation?.Guest?.firstName
    || reservation?.guest
  ...
  if (Array.isArray(rooms)) {
    if (emittedRoomNumber) {
      roomObj = rooms.find(r => r.number === emittedRoomNumber || r.roomNumber === emittedRoomNumber)
    }
  ...
  const category: string | undefined = (roomObj as any)?.type || (roomObj as any)?.roomType || (roomObj as any)?.RoomType?.typeName
