...
import { getMinPhoneLength, getMaxPhoneLength, getCountryName, isValidPhoneForCountry, formatPhoneByCountry } from '@/utils/phone'
...
export const useReservationForm = () => {
  ...
  const isFormValid = computed(() => {
    ...
    const hasRequired = !!(f.firstName && f.lastName && f.email && f.phone && f.address && f.idDocument && f.numGuest > 0 && f.checkIn && f.checkOut && f.roomNumber)
  ...
  const validateFirstName = () => {
    ...
    if (v.length < 2) return (errors.value.firstName = 'First name must be at least 2 characters', false)
    if (v.length > 50) return (errors.value.firstName = 'First name must not exceed 50 characters', false)
    if (!/^[a-zA-Z\s'-]+$/.test(v)) return (errors.value.firstName = "First name can only contain letters, spaces, hyphens, and apostrophes", false)
    return true
  }
  ...
  const validateLastName = () => {
    ...
    if (v.length < 2) return (errors.value.lastName = 'Last name must be at least 2 characters', false)
    if (v.length > 50) return (errors.value.lastName = 'Last name must not exceed 50 characters', false)
    if (!/^[a-zA-Z\s'-]+$/.test(v)) return (errors.value.lastName = "Last name can only contain letters, spaces, hyphens, and apostrophes", false)
    return true
  }
  ...
  const validateMiddleName = () => {
    ...
    if (v) {
      if (v.length > 50) return (errors.value.middleName = 'Middle name must not exceed 50 characters', false)
      if (!/^[a-zA-Z\s'-]+$/.test(v)) return (errors.value.middleName = "Middle name can only contain letters, spaces, hyphens, and apostrophes", false)
    }
  ...
  const validateEmail = () => {
    ...
    if (!emailRegex.test(email)) return (errors.value.email = 'Please enter a valid email address', false)
    if (email.length > 100) return (errors.value.email = 'Email must not exceed 100 characters', false)
    return true
  }
  ...
  const validateIdDocument = () => {
    ...
    if (v.length < 5) return (errors.value.idDocument = 'ID document must be at least 5 characters', false)
    if (v.length > 20) return (errors.value.idDocument = 'ID document must not exceed 20 characters', false)
    if (!/^[A-Z0-9\s-]+$/.test(v)) return (errors.value.idDocument = 'ID document can only contain letters, numbers, spaces, and hyphens', false)
    return true
  }
  ...
  const validateAddress = () => {
    ...
    if (v.length < 10) return (errors.value.address = 'Address must be at least 10 characters', false)
    if (v.length > 200) return (errors.value.address = 'Address must not exceed 200 characters', false)
    return true
  }
  ...
  // Room selection validation should be combined with availability check by caller
  const validateRoomSelection = (isRoomAvailable?: (roomNumber: string) => boolean) => {
    ...
    if (isRoomAvailable && !isRoomAvailable(rn)) {
      return (errors.value.roomNumber = 'Selected room is not available for the chosen dates', false)
    }
    return true
  }

  // Phone validators/formatters
  const validatePhone = () => {
    ...
    if (digits.length < minLen) return (errors.value.phone = `Phone number must be at least ${minLen} digits for ${getCountryName(cc)}` , false)
    if (digits.length > maxLen) return (errors.value.phone = `Phone number must not exceed ${maxLen} digits for ${getCountryName(cc)}` , false)
    if (!isValidPhoneForCountry(digits, cc)) return (errors.value.phone = `Invalid phone number format for ${getCountryName(cc)}`, false)
    return true
  }

  // Handlers (no side effects like autosave here)
  ...
  const handleEmailKeydown = (event: KeyboardEvent) => {
    const email = formData.value.email.trim()
    if ((event.key === 'Tab' || event.key === ' ' || event.key === 'Enter') && email && !email.includes('@') && !email.includes('.')) {
