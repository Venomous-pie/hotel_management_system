...
export const useRoomAvailability = (
  ...
  const selectedRoom = computed(() => rooms.value.find(r => r.number === formData.value.roomNumber))
  const selectedRoomPrice = computed(() => selectedRoom.value?.pricePerNight || 0)
  const calculatedPrice = computed(() => (nights.value > 0 && selectedRoomPrice.value > 0) ? (nights.value * selectedRoomPrice.value).toFixed(2) : '0.00')
  ...
  const filterAvailableRooms = () => {
    if (formData.value.checkIn && formData.value.checkOut && formData.value.numGuest) {
      ...
      setTimeout(() => { isCheckingAvailability.value = false }, 500)
    }
  }
